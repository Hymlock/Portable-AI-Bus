'use strict';
/**
 * AUDIT round 4h — grok, this wake. Own instrument against dist/ + scripts/claim-guard-cli.js.
...
async function seedRepo() {
  const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r4h-'));
...
async function item15() {
...

  {
    const fx = await seedRepo();
    try {
      if (!await plantOwnModules(fx.repo)) rec(15, 'rootdirs-nm-junction', 'SKIP', 'no modules');
      else {
        const leakSrc = path.join(fx.repo, 'node_modules', 'leak-src');
        if (!junction(leakSrc, path.join(fx.repo, 'src'))) {
          rec(15, 'rootdirs-nm-junction', 'SKIP', 'no junction');
        } else {
          await fsp.writeFile(path.join(fx.repo, 'tsconfig.json'), JSON.stringify({
            compilerOptions: {
              strict: true, noEmit: true, skipLibCheck: true, types: [],
              rootDir: 'src',
              rootDirs: ['src', 'node_modules/leak-src']
            },
            include: ['src', 'node_modules/leak-src']
          }, null, 2));
          await fsp.writeFile(path.join(fx.repo, 'src', 'index.ts'), 'export const broken: number = "no";\n');
          git(fx.repo, 'add', '-A');
          await fsp.writeFile(path.join(fx.repo, 'src', 'index.ts'), 'export const good: number = 1;\n');
          const result = runGuard(fx.repo, fx.busRoot);
          rec(15, 'rootdirs-nm-junction',
            compileOk(result) ? 'FAIL' : refused(result) ? 'PASS' : 'FAIL',
            `code=${result.code} compileOk=${compileOk(result)} out=${result.out.trim().slice(0, 600)}`);
        }
      }
      ...
      if (!await linkWholeModules(fx.repo)) rec(15, 'tsBuildInfoFile-outside', 'SKIP', 'no node_modules');
        ...
        await fsp.writeFile(path.join(fx.repo, 'tsconfig.json'), JSON.stringify({
          compilerOptions: {
            ...
            tsBuildInfoFile: path.join(os.tmpdir(), `pab-r4h-leak-${process.pid}.tsbuildinfo`).replace(/\\/g, '/')
          },
          include: ['src']
        }, null, 2));
        git(fx.repo, 'add', '-A');
...
async function item2() {
  async function withRoot(fn) {
    const root = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r4h-e-'));
    try {
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r4h-mb-'));
    try {
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r4h-mb2-'));
    try {
      const store = new MailboxStore(dir);
      await store.ensureInitialized(['claude', 'grok'], 500);
      const source = await store.send({ from: 'claude', to: 'grok', kind: 'task', subject: 'work', body: 'do it' });
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r4h-mb3-'));
    try {
      const store = new MailboxStore(dir);
      await store.ensureInitialized(['claude', 'grok'], 500);
      const source = await store.send({ from: 'claude', to: 'grok', kind: 'task', subject: 'work', body: 'do it' });
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r4h-mb4-'));
    try {
      const store = new MailboxStore(dir);
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r4h-cli-'));
    try {
      const store = new MailboxStore(dir);
      await store.ensureInitialized(['claude', 'grok'], 500);
      const source = await store.send({ from: 'claude', to: 'grok', kind: 'task', subject: 'work', body: 'do it' });
      ...
      const records = await store.listEvidence(source.seq);
      const summaries = records.filter((r) => r.consolidatedFrom !== undefined);
      rec(2, 'cli-consolidate-evidence-reaches-store',
        code === 0 && summaries.length === 1 && summaries[0].consolidatedFrom.length === 3 ? 'PASS' : 'FAIL',
        `code=${code} summaries=${summaries.length} out=${String(out).trim().slice(0, 300)}`);
    } finally { await rm(dir); }
  }

  {
    const root = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r4h-rec-'));
    try {
  ...
  {
    const root = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r4h-con-'));
    try {
...
async function item20() {
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r4h-20a-'));
    try {
      const store = new MailboxStore(dir);
      await store.ensureInitialized(['claude', 'grok'], 500);
      const source = await store.send({ from: 'claude', to: 'grok', kind: 'task', subject: 'work', body: 'do it' });
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r4h-20b-'));
    try {
      const store = new MailboxStore(dir);
      await store.ensureInitialized(['claude', 'grok'], 500);
      const source = await store.send({ from: 'claude', to: 'grok', kind: 'task', subject: 'work', body: 'do it' });
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r4h-20c-'));
    try {
      const store = new MailboxStore(dir);
      await store.ensureInitialized(['claude', 'grok'], 500);
      const source = await store.send({ from: 'claude', to: 'grok', kind: 'task', subject: 'work', body: 'do it' });
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r4h-20d-'));
    try {
      const store = new MailboxStore(dir);
      await store.ensureInitialized(['claude', 'grok'], 500);
      const source = await store.send({ from: 'claude', to: 'grok', kind: 'task', subject: 'work', body: 'do it' });
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r4h-20e-'));
    try {
      const store = new MailboxStore(dir);
      await store.ensureInitialized(['claude', 'grok'], 500);
      const source = await store.send({ from: 'claude', to: 'grok', kind: 'task', subject: 'work', body: 'do it' });
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r4h-20f-'));
    try {
      const store = new MailboxStore(dir);
      await store.ensureInitialized(['claude', 'grok'], 500);
      const source = await store.send({ from: 'claude', to: 'grok', kind: 'task', subject: 'work', body: 'do it' });
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r4h-20cli-'));
    try {
      const store = new MailboxStore(dir);
      await store.ensureInitialized(['claude', 'grok'], 500);
      const source = await store.send({ from: 'claude', to: 'grok', kind: 'task', subject: 'work', body: 'do it' });
...
(async () => {
  ...
  await fsp.writeFile(path.join(REPO, 'tmp-audit-r4h-grok-out.json'), JSON.stringify(out, null, 2));
