'use strict';
/**
 * Independent AUDIT instrument (grok), this session, round 3k.
...
async function claimFixture() {
  const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r3k-13-'));
...
async function recallFixture() {
  const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r3k-10-'));
...
async function attack10() {
...

  {
    ...
    const tmp = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r3k-10cli-'));
    try {
...
async function attack18() {
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r3k-18x-'));
    try {
      const store = new MailboxStore(dir);
      await store.ensureInitialized(['claude', 'grok', 'codex'], 500);
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r3k-18c-'));
    try {
      const store = new MailboxStore(dir);
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r3k-18t-'));
    try {
      const store = new MailboxStore(dir);
      await store.ensureInitialized(['claude', 'grok'], 500);
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r3k-18r-'));
    try {
      const store = new MailboxStore(dir);
      await store.ensureInitialized(['claude', 'grok'], 500);
      const original = await store.send({ from: 'claude', to: 'grok', kind: 'task', subject: 'stale', body: 'old' });
...
async function seedRepo() {
  const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r3k-15-'));
...
async function main() {
  ...
  const outPath = path.join(REPO, 'tmp-audit-r3k-grok-out.json');
