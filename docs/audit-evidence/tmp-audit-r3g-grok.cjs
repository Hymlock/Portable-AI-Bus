 ...
 * Brief: 8ea4c35 / 082ddfa. HEAD may be later; say it first.
 * Read src/, attack dist/ + scripts/claim-guard-cli.js. Do not edit src/ or tests/.
 * Item 2 left alone.
 */
...
async function attack13() {
  // Named: symlink rather than junction, to the parent.
  {
    ...
    try {
      if (process.platform !== 'win32' || !dirSymlink(path.join(fx.workspace, 'above'), fx.dir)) {
        ...
        const other = await fx.store.claim({ agent: 'claude', paths: ['src/bus.ts'], why: 'still free', repoRoot: fx.workspace });
        rec(13, 'symlink-to-parent', refused && other ? 'PASS' : 'FAIL',
          refused
            ? `refused (${why}); other seat still claimed src/bus.ts`
            : `ACCEPTED a symlink to the parent; other=${Boolean(other)}`);
      }
    } finally {
      await rm(fx.dir);
    }
  ...
  // Named: a root that is itself a link.
  {
    const fx = await claimFixture();
    try {
  ...
  // Named: case differences on Windows.
  {
    const fx = await claimFixture();
    try {
  ...
  // Named: path that resolves under a root but still blocks the tree.
  {
    const fx = await claimFixture();
    try {
  ...
  // Extra: file symlink alias to a file above the root.
  {
    const fx = await claimFixture();
    try {
  ...
  // Extra: missing root refuses before path resolution.
  {
    const fx = await claimFixture();
    try {
      let refused = false;
  ...
  // Extra: src/../src/bus.ts still an ordinary under-root claim.
  {
    const fx = await claimFixture();
    try {
      const held = await fx.store.claim({
...
async function attack10() {
  // Named: inheritedFrom is not an authority.
  {
    ...
    try {
      ...
      const row = (await fx.store.listEvidence?.()) || [];
      void row;
  ...
  // Named: a checkpoint on a DIFFERENT workId does not grant this one.
  {
    const fx = await recallFixture();
    try {
      ...
      try {
        ...
        opened = false;
        rec(10, 'different-workId', 'NOTE', `openRecovery(#2) after move of #1: ${errMsg(error)}`);
      }
      ...
      rec(10, 'different-workId', !leaked && !resurrected && (twoOk || !opened) ? 'PASS' : 'FAIL',
        `leak-before-open=${Boolean(leaked)} resurrected-#1=${Boolean(resurrected)} #2=${twoOk ? 'BRIEF' : 'undefined'}`);
    } finally {
      await rm(fx.dir);
    }
  }

  // Named: reassignBaton twice.
  {
    const fx = await recallFixture();
    try {
  ...
  // Extra: superseded source is not recalled even with an open checkpoint.
  {
    const fx = await recallFixture();
    try {
      await fx.store.send({
  ...
  // Extra: holder still recalls after consume (checkpoint is the revocation path).
  {
    const fx = await recallFixture();
    try {
      ...
      rec(10, 'consumed-still-recalled-while-open', still ? 'PASS' : 'FAIL',
        still ? 'open checkpoint still recalls a consumed brief (revocation is close)' : 'consume revoked recall; that would be a different product');
    } finally {
      await rm(fx.dir);
    }
  }

  // PRODUCTION: is recallAssignment wired on cliBusClient?
  {
    ...
    rec(10, 'production-source-mentions-recall',
      srcHas || distHas ? 'PASS' : 'FAIL',
      `bus-client src=${srcHas} dist=${distHas} cliUsesCliBusClient=${usesCli} runnerGuards=${optional}`);

    const tmp = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r3g-10cli-'));
    try {
...
async function attack18() {
  ...
  // Cross-recipient: both verbs refuse; atomic sends nothing.
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r3g-18x-'));
    try {
      const store = new MailboxStore(dir);
      await store.ensureInitialized(['claude', 'grok', 'codex'], 500);
  ...
  // Consumed target: neither verb sets supersededBy.
  {
    ...
    try {
      ...
      const reread = JSON.parse(
        await fsp.readFile(await findMessageFile(dir, original.seq), 'utf8')
      );
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r3g-18t-'));
    try {
      const store = new MailboxStore(dir);
      await store.ensureInitialized(['claude', 'grok'], 500);
      ...
      rec(18, 'atomic-writes-supersededAt',
        correction.superseded === true && raw.supersededBy === correction.seq && raw.supersededAt && raw.supersedeReason === 'settled already'
          ? 'PASS' : 'FAIL',
        `supersededAt=${raw.supersededAt || 'MISSING'} by=${raw.supersededBy}`);
    } finally {
      await rm(dir);
    }
  }

  // Remaining disagreement: empty reason.
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r3g-18r-'));
    try {
      const store = new MailboxStore(dir);
      await store.ensureInitialized(['claude', 'grok'], 500);
      ...
      let twoStepEmpty = 'threw?';
      try {
        await store.supersedeMessage(a.seq, b.seq, '   ', 'claude');
        twoStepEmpty = 'accepted';
      } catch (error) {
        twoStepEmpty = /reason must not be empty/i.test(errMsg(error)) ? 'refused' : errMsg(error);
      }
      ...
      rec(18, 'empty-reason-disagreement',
        'NOTE',
        `two-step empty reason: ${twoStepEmpty}; atomic whitespace reason retracted=${atomic.superseded} storedReason=${raw.supersedeReason}`);
    } finally {
      await rm(dir);
    }
  }
...
async function plantLeakPackage(repo, includeAbs) {
  ...
  await fsp.writeFile(path.join(pkg, 'tsconfig.json'), JSON.stringify({
    compilerOptions: { strict: true, noEmit: true, skipLibCheck: true, types: [] },
    include: [includeAbs.replace(/\\/g, '/')]
  }, null, 2));
}

async function attack15() {
  // Negative control: staged type error is refused and names TS2322.
  {
    const fx = await fixtureRepo();
    try {
      if (!fx.linked) {
        rec(15, 'staged-type-error-refused', 'SKIP', 'could not link node_modules');
      } else {
        await fsp.writeFile(path.join(fx.repo, 'src', 'index.ts'), 'export const broken: number = "no";\n');
        git(fx.repo, 'add', '-A');
        const result = runGuard(fx.repo, fx.busRoot);
        rec(15, 'staged-type-error-refused',
          result.code === 1 && /TS2322|does not compile/.test(result.out) ? 'PASS' : 'FAIL',
  ...
  // Honest green control.
  {
    ...
    try {
      if (!fx.linked) {
        rec(15, 'honest-commit-green', 'SKIP', 'could not link node_modules');
      } else {
        git(fx.repo, 'add', '-A');
        const result = runGuard(fx.repo, fx.busRoot);
  ...
  // Closed: absolute include into the worktree.
  {
    const fx = await fixtureRepo();
    try {
      if (!fx.linked) {
        ...
        rec(15, 'absolute-include-worktree',
          result.code === 1 && /OUTSIDE the staged tree/.test(result.out) ? 'PASS' : 'FAIL',
          `exit=${result.code} ${result.out}`);
      }
    } finally {
      await rm(fx.dir);
  ...
  // Closed: extends an untracked worktree-only base.
  {
    const fx = await fixtureRepo();
    try {
      if (!fx.linked) {
        rec(15, 'extends-untracked-base', 'SKIP', 'could not link node_modules');
      } else {
        await fsp.writeFile(path.join(fx.repo, 'src', 'index.ts'), 'export const broken: number = "no";\n');
        ...
        const result = runGuard(fx.repo, fx.busRoot);
        rec(15, 'extends-untracked-base',
          result.code === 1 && /OUTSIDE the staged tree|extends/.test(result.out) ? 'PASS' : 'FAIL',
          `exit=${result.code} ${result.out}`);
      }
    } finally {
      await rm(fx.dir);
  ...
  // Hatch is loud.
  {
    ...
    try {
      if (!fx.linked) {
        rec(15, 'hatch-loud', 'SKIP', 'could not link node_modules');
      } else {
        await fsp.writeFile(path.join(fx.repo, 'src', 'index.ts'), 'export const broken: number = "no";\n');
        git(fx.repo, 'add', '-A');
        const result = runGuard(fx.repo, fx.busRoot, { BUS_ALLOW_BROKEN_BUILD: '1' });
        rec(15, 'hatch-loud',
          result.code === 0 && /SKIPPED/.test(result.out) ? 'PASS' : 'FAIL',
          `exit=${result.code} ${result.out}`);
      }
    } finally {
      await rm(fx.dir);
  ...
  // THE HOLE: bare extends of a local package whose include is the worktree.
  async function bareExtends(label, extendsValue) {
    const fx = await fixtureRepo();
    try {
      if (!fx.linked) {
        rec(15, label, 'SKIP', 'could not link node_modules');
        return;
      }
      await fsp.writeFile(path.join(fx.repo, 'src', 'index.ts'), 'export const broken: number = "no";\n');
      await fsp.writeFile(path.join(fx.repo, 'tsconfig.json'), JSON.stringify({
      ...
      git(fx.repo, 'add', 'src', 'tsconfig.json', '.gitignore');
      await fsp.writeFile(path.join(fx.repo, 'src', 'index.ts'), 'export const good: number = 1;\n');
      await plantLeakPackage(fx.repo, path.join(fx.repo, 'src'));
      const result = runGuard(fx.repo, fx.busRoot);
      rec(15, label,
  ...
  await bareExtends('bare-extends-package-file', 'leak-config/tsconfig.json');
  await bareExtends('bare-extends-array', ['leak-config/tsconfig.json']);
  await bareExtends('bare-extends-package-name', 'leak-config');

  // Contrast: relative extends of the same package MUST be walked and refused.
  {
    const fx = await fixtureRepo();
    try {
      if (!fx.linked) {
        rec(15, 'relative-extends-same-package', 'SKIP', 'could not link node_modules');
      } else {
        await fsp.writeFile(path.join(fx.repo, 'src', 'index.ts'), 'export const broken: number = "no";\n');
        await fsp.writeFile(path.join(fx.repo, 'tsconfig.json'), JSON.stringify({
        ...
        rec(15, 'relative-extends-same-package',
          result.code === 1 && /OUTSIDE the staged tree|include/.test(result.out) ? 'PASS' : 'FAIL',
          `exit=${result.code} ${result.out}`);
      }
    } finally {
      await rm(fx.dir);
  ...
  // Control: same bare extends, worktree ALSO broken — tsc names the WORKTREE path.
  {
    const fx = await fixtureRepo();
    try {
      if (!fx.linked) {
        rec(15, 'bare-extends-worktree-also-broken', 'SKIP', 'could not link node_modules');
      } else {
        await fsp.writeFile(path.join(fx.repo, 'src', 'index.ts'), 'export const broken: number = "no";\n');
        await fsp.writeFile(path.join(fx.repo, 'tsconfig.json'), JSON.stringify({
        ...
        await plantLeakPackage(fx.repo, path.join(fx.repo, 'src'));
        const result = runGuard(fx.repo, fx.busRoot);
        const namesWorktree = result.out.includes(fx.repo.replace(/\\/g, '/'))
          || result.out.includes(fx.repo)
          || /src[\\/]index\.ts/.test(result.out);
        rec(15, 'bare-extends-worktree-also-broken',
          result.code === 1 && namesWorktree ? 'PASS' : (result.code === 1 ? 'NOTE' : 'FAIL'),
          `exit=${result.code} namesWorktree=${namesWorktree} ${result.out}`);
      }
    } finally {
      await rm(fx.dir);
    }
  ...
  // Same class: include a node_modules junction that portals to worktree src.
  {
    const fx = await fixtureRepo();
    try {
      if (!fx.linked) {
        rec(15, 'node-modules-junction-include', 'SKIP', 'could not link node_modules');
      } else {
        await fsp.writeFile(path.join(fx.repo, 'src', 'index.ts'), 'export const broken: number = "no";\n');
        const portal = path.join(fx.repo, 'node_modules', 'leak-src');
        const ported = junction(portal, path.join(fx.repo, 'src'));
        await fsp.writeFile(path.join(fx.repo, 'tsconfig.json'), JSON.stringify({
          compilerOptions: { strict: true, noEmit: true, skipLibCheck: true, types: [] },
        ...
        rec(15, 'node-modules-junction-include',
          !ported ? 'SKIP' : (result.code === 1 ? 'PASS' : 'FAIL'),
          `ported=${ported} exit=${result.code} ${result.out}`);
      }
    } finally {
      await rm(fx.dir);
    }
  ...
  // New: ${configDir}/../  — walker treats ${configDir} as a folder name.
  {
    const fx = await fixtureRepo();
    try {
      if (!fx.linked) {
        rec(15, 'configDir-parent-escape', 'SKIP', 'could not link node_modules');
      } else {
        await fsp.writeFile(path.join(fx.repo, 'src', 'index.ts'), 'export const broken: number = "no";\n');
        await fsp.writeFile(path.join(fx.repo, 'tsconfig.json'), JSON.stringify({
          compilerOptions: { strict: true, noEmit: true, skipLibCheck: true, types: [] },
        ...
        await fsp.writeFile(path.join(fx.repo, 'src', 'index.ts'), 'export const good: number = 1;\n');
        const result = runGuard(fx.repo, fx.busRoot);
        rec(15, 'configDir-parent-escape',
          result.code === 1 ? 'PASS' : 'NOTE',
          `exit=${result.code} ${result.out}`);
      }
    } finally {
      await rm(fx.dir);
  ...
  // Walker source: confirm the bare-specifier skip is still written.
  {
    const cli = fs.readFileSync(GUARD, 'utf8');
    const skip = /if\s*\(\s*!isRelative\s*\)\s*continue/.test(cli);
    rec(15, 'walker-skips-bare-extends',
      skip ? 'FAIL' : 'PASS',
      skip
        ? 'findConfigEscapes still does if (!isRelative) continue on extends — the written exception'
        : 'bare extends are no longer skipped');
  }
}

async function main() {
  const head = execFileSync('git', ['rev-parse', 'HEAD'], { cwd: REPO, encoding: 'utf8' }).trim();
  const log = execFileSync('git', ['log', '--oneline', '-8'], { cwd: REPO, encoding: 'utf8' }).trim();
  ...
  const out = {
    ...
    summary,
    results,
    byItem: [13, 10, 18, 15].map((item) => ({
      item,
      fail: results.filter((r) => r.item === item && r.status === 'FAIL').map((r) => r.name),
      pass: results.filter((r) => r.item === item && r.status === 'PASS').map((r) => r.name),
      note: results.filter((r) => r.item === item && r.status === 'NOTE').map((r) => r.name)
    }))
  };
  await fsp.writeFile(path.join(REPO, 'tmp-audit-r3g-grok-out.json'), JSON.stringify(out, null, 2));
  console.log('\nSUMMARY', JSON.stringify(summary));
  console.log(JSON.stringify(out.byItem, null, 2));
}
