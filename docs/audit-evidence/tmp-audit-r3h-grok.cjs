'use strict';
/**
 * Independent AUDIT instrument (grok), this session, round 3.
 * Brief named 8ea4c35 / 082ddfa. Report HEAD first. Attack dist/ + scripts/.
...
async function claimFixture() {
  const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r3h-13-'));
...
async function recallFixture() {
  const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r3h-10-'));
...
async function attack10() {
...

  {
    ...
    const tmp = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r3h-10cli-'));
    try {
...
async function attack18() {
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r3h-18x-'));
    try {
      const store = new MailboxStore(dir);
      await store.ensureInitialized(['claude', 'grok', 'codex'], 500);
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r3h-18c-'));
    try {
      const store = new MailboxStore(dir);
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r3h-18t-'));
    try {
      const store = new MailboxStore(dir);
      await store.ensureInitialized(['claude', 'grok'], 500);
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r3h-18r-'));
    try {
      const store = new MailboxStore(dir);
      await store.ensureInitialized(['claude', 'grok'], 500);
      const original = await store.send({ from: 'claude', to: 'grok', kind: 'task', subject: 'stale', body: 'old' });
...
async function seedRepo() {
  const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r3h-15-'));
...
async function main() {
  ...
  const outPath = path.join(REPO, 'tmp-audit-r3h-grok-out.json');
