'use strict';
/**
 * Independent AUDIT instrument (grok), this session, round 3l.
 * Brief named 8ea4c35 / 082ddfa. Report HEAD first.
 * Attack dist/ + scripts/claim-guard-cli.js. Do not edit src/ or tests/. Item 2 left alone.
 *
 * This is not a re-run of r3k. Same named attacks Claude asked for, plus one extra
 * 15-control (paths-to-worktree must still refuse) so a green walker cannot hide
 * behind "I only ran the author's suite."
 */
...
async function claimFixture() {
  const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r3l-13-'));
...
async function attack13() {
...

  {
    ...
    try {
      const held = await fx.store.claim({
        agent: 'claude', paths: ['src/'], why: 'trailing slash', repoRoot: fx.workspace
      });
      rec(13, 'trailing-slash-dir', held ? 'PASS' : 'FAIL',
        held ? 'src/ accepted as an ancestor under the root' : 'trailing slash refused a legal directory');
    } catch (error) {
      rec(13, 'trailing-slash-dir', 'FAIL', errMsg(error));
    } finally {
      await rm(fx.dir);
    }
  }

  {
    const fx = await claimFixture();
    try {
      const foreign = path.join(fx.dir, 'foreign-tree');
      await fsp.mkdir(path.join(foreign, 'src'), { recursive: true });
      await fsp.writeFile(path.join(foreign, 'src', 'secret.ts'), 'x');
      if (!junction(path.join(fx.workspace, 'looks-local'), foreign)) {
        rec(13, 'junction-to-foreign-tree', 'SKIP', 'junction unavailable');
      } else {
        let refused = false;
        let why = '';
        try {
          await fx.store.claim({
            agent: 'codex', paths: ['looks-local'], why: 'foreign via junction', repoRoot: fx.workspace
          });
        } catch (error) {
          refused = /whole repositor|too broad/i.test(errMsg(error));
          why = errMsg(error);
        }
        rec(13, 'junction-to-foreign-tree', refused ? 'PASS' : 'FAIL',
          refused ? why : 'ACCEPTED a junction whose realpath is a foreign tree');
      }
    } finally {
      await rm(fx.dir);
    }
...
async function recallFixture() {
  const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r3l-10-'));
...
async function attack10() {
...

  {
    ...
    try {
      await fx.store.reassignBaton({ to: 'codex', reason: 'moved', force: true });
      const closed = await fx.store.operatorCloseRecovery('codex', fx.source.seq, 'stale after death');
      const afterClose = await fx.store.recallAssignment('codex', fx.source.seq);
      const predAfterClose = await fx.store.recallAssignment('grok', fx.source.seq);
      let reopened = false;
      let reopenErr = '';
      try {
        await fx.store.openRecovery('grok', fx.source.seq, 'addressee after operator close');
        reopened = true;
      } catch (error) {
        reopenErr = errMsg(error);
      }
      const resurrected = reopened ? await fx.store.recallAssignment('grok', fx.source.seq) : undefined;
      rec(10, 'operator-close-then-addressee-reopen',
        closed && !afterClose && !predAfterClose ? 'NOTE' : 'FAIL',
        `closed=${Boolean(closed)} succRecallAfter=${Boolean(afterClose)} predRecallAfter=${Boolean(predAfterClose)} addresseeReopened=${reopened} resurrected=${Boolean(resurrected)} err=${reopenErr}. Address is still the first-open authority once no holder remains.`);
    } finally {
      await rm(fx.dir);
    }
  }

  {
    ...
    const tmp = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r3l-10cli-'));
    try {
...
async function attack18() {
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r3l-18x-'));
    try {
      const store = new MailboxStore(dir);
      await store.ensureInitialized(['claude', 'grok', 'codex'], 500);
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r3l-18c-'));
    try {
      const store = new MailboxStore(dir);
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r3l-18t-'));
    try {
      const store = new MailboxStore(dir);
      await store.ensureInitialized(['claude', 'grok'], 500);
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r3l-18r-'));
    try {
      const store = new MailboxStore(dir);
      await store.ensureInitialized(['claude', 'grok'], 500);
      const original = await store.send({ from: 'claude', to: 'grok', kind: 'task', subject: 'stale', body: 'old' });
      ...
      try {
        ...
        twoStepEmpty = true;
        twoStepWhy = errMsg(error);
      }
      rec(18, 'empty-reason-disagreement',
        'NOTE',
        `atomic whitespace reason defaulted to "${raw.supersedeReason}" and retracted=${Boolean(raw.supersededBy)}; two-step threw=${twoStepEmpty} (${twoStepWhy}). Shape disagreement, not who-may-retract.`);
    } finally {
      await rm(dir);
    }
  }
...
async function seedRepo() {
  const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r3l-15-'));
...
async function main() {
  ...
  const outPath = path.join(REPO, 'tmp-audit-r3l-grok-out.json');
