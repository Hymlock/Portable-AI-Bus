'use strict';
/**
 * AUDIT round 4e — grok's independent instrument (this wake).
 * Attacks compiled dist/ + scripts/claim-guard-cli.js. Does not edit src/ or tests/.
 * Stopping rule (adopted): VARIANT = new spelling of a decided rule.
...
async function seedRepo() {
  const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r4e-'));
...
async function item15() {
  ...
  for (const [name, options] of [
    ['tsBuildInfoFile-outside', { incremental: true, tsBuildInfoFile: path.join(os.tmpdir(), `pab-r4e-leak-${process.pid}.tsbuildinfo`).replace(/\\/g, '/') }],
    ['outFile-outside', { outFile: path.join(os.tmpdir(), `pab-r4e-out-${process.pid}.js`).replace(/\\/g, '/'), module: 'amd' }],
    ['mapRoot-outside', { sourceMap: true, mapRoot: path.join(os.tmpdir(), 'pab-r4e-maps').replace(/\\/g, '/') }]
  ]) {
    const fx = await seedRepo();
    try {
...
async function item2() {
  async function withRoot(fn) {
    const root = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r4e-e-'));
    try {
  ...
  for (const [name, contents] of [
    ['empty-lock-is-debris', ''],
    ['malformed-lock-is-debris', '{not-json'],
    ['missing-pid-lock-is-debris', JSON.stringify({ at: new Date().toISOString() })]
  ]) {
    await withRoot(async (store, root) => {
      const lockPath = path.join(root, '.ai-bus', 'runtime', 'mailbox', 'evidence.json.lock');
      await fsp.mkdir(path.dirname(lockPath), { recursive: true });
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r4e-mb-'));
    try {
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r4e-mb2-'));
    try {
      const store = new MailboxStore(dir);
      await store.ensureInitialized(['claude', 'grok'], 500);
      const source = await store.send({ from: 'claude', to: 'grok', kind: 'task', subject: 'work', body: 'do it' });
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r4e-mb3-'));
    try {
      const store = new MailboxStore(dir);
      await store.ensureInitialized(['claude', 'grok'], 500);
      const source = await store.send({ from: 'claude', to: 'grok', kind: 'task', subject: 'work', body: 'do it' });
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r4e-mb4-'));
    try {
      const store = new MailboxStore(dir);
  ...
  {
    const root = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r4e-rec-'));
    try {
  ...
  {
    const root = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r4e-con-'));
    try {
  ...
  {
    const root = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r4e-ul-'));
    try {
      const dir = path.join(root, '.ai-bus', 'runtime', 'mailbox');
      await fsp.mkdir(dir, { recursive: true });
      const filler = 'x'.repeat(2048);
...
async function item20() {
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r4e-20a-'));
    try {
      const store = new MailboxStore(dir);
      await store.ensureInitialized(['claude', 'grok'], 500);
      const source = await store.send({ from: 'claude', to: 'grok', kind: 'task', subject: 'work', body: 'do it' });
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r4e-20b-'));
    try {
      const store = new MailboxStore(dir);
      await store.ensureInitialized(['claude', 'grok'], 500);
      const source = await store.send({ from: 'claude', to: 'grok', kind: 'task', subject: 'work', body: 'do it' });
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r4e-20c-'));
    try {
      const store = new MailboxStore(dir);
      await store.ensureInitialized(['claude', 'grok'], 500);
      const source = await store.send({ from: 'claude', to: 'grok', kind: 'task', subject: 'work', body: 'do it' });
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r4e-20d-'));
    try {
      const store = new MailboxStore(dir);
      await store.ensureInitialized(['claude', 'grok'], 500);
      const source = await store.send({ from: 'claude', to: 'grok', kind: 'task', subject: 'work', body: 'do it' });
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r4e-20e-'));
    try {
      const store = new MailboxStore(dir);
      await store.ensureInitialized(['claude', 'grok'], 500);
      const source = await store.send({ from: 'claude', to: 'grok', kind: 'task', subject: 'work', body: 'do it' });
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r4e-20f-'));
    try {
      const store = new MailboxStore(dir);
      await store.ensureInitialized(['claude', 'grok'], 500);
      const source = await store.send({ from: 'claude', to: 'grok', kind: 'task', subject: 'work', body: 'do it' });
  ...
  {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'pab-r4e-20cli-'));
    try {
      const store = new MailboxStore(dir);
      await store.ensureInitialized(['claude', 'grok'], 500);
      const source = await store.send({ from: 'claude', to: 'grok', kind: 'task', subject: 'work', body: 'do it' });
...
(async () => {
  ...
  await fsp.writeFile(path.join(REPO, 'tmp-audit-r4e-grok-out.json'), JSON.stringify(out, null, 2));
