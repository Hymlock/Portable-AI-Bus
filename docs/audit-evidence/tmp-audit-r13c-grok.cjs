...
'use strict';
/**
 * r13c — item 2 hole 1 re-measured on the patched dist with the REAL lock path.
 * r6/r13 wrote <root>/evidence.json.lock; EvidenceStore locks
 * <root>/.ai-bus/runtime/mailbox/evidence.json.lock. Those earlier PASSes
 * were instrument errors. Not a re-run of r7 (that was HEAD).
 */
...
async function withRoot(name, fn) {
  const dir = await fsp.mkdtemp(path.join(os.tmpdir(), `pab-r13c-${name}-`));
  try {
...
async function main() {
  ...
  await fsp.writeFile(path.join(REPO, 'tmp-audit-r13c-grok-out.json'), JSON.stringify(summary, null, 2));
